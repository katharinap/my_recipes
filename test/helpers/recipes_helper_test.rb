...
class RecipesHelperTest < ActionView::TestCase
  ...
  context '.new_link' do
    should 'return the proper markup when user_signed_in? == false' do
      stubs(:user_signed_in?).returns(false)
      expected_markup =
          "<a href=\"/recipes/new\"><button type=\"button\" class=\"btn btn-success-outline btn-sm hidden-print disabled\" disabled=\"disabled\">Add Recipe</button></a>"
    ...
    should 'return the proper markup when user_signed_in? == true' do
      stubs(:user_signed_in?).returns(true)
      expected_markup =
          "<a href=\"/recipes/new\"><button type=\"button\" class=\"btn btn-success-outline btn-sm hidden-print\">Add Recipe</button></a>"
      assert_equal expected_markup, new_link
    end
  end
